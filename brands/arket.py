...
def get_arket_product(url: str) -> dict:
    ...
    try:
        ...
        cookies = {
            'bm_s': '',
        }
        ...
        headers = {
            ...
            'priority': 'u=0, i',
            'referer': 'https://www.arket.com/en-ww/',
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if not scripts:
                    raise ValueError(
                        'No JSON-LD product data found on the page.')
                ...
                source = next(
                    (item for item in product_graph if item.get('@type') == 'Product'), None)
                if source is None:
                    raise ValueError(
                        'No Product entry found in the JSON-LD data.')
                ...
                if attempt == MAX_RETRIES:
                    raise
                print(f'[yellow]Arket attempt {attempt}/{MAX_RETRIES} failed: {exc}. Retrying...[/yellow]')
                print(f'[yellow]Waiting {PROXY_SPIN_TIMEOUT} seconds before retrying...[/yellow]')
        ...
        thumbnail_url = f"{BASE_URL.rstrip('/')}{STATIC_ROUTE}/{thumb_filename}" if thumb_filename else ''
